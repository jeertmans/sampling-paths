...
def main() -> None:
    ...
    parser.add_argument(
        "--optim",
        type=str,
        default="optax.chain(optax.zero_nans(),optax.contrib.muon(learning_rate))",
    ...
    parser.add_argument(
        ...
        default=False,
        action=argparse.BooleanOptionalAction,
        help="Whether to print the model configuration and exit.",
    )
    parser.add_argument(
        "--print-agent-config",
        default=False,
        action=argparse.BooleanOptionalAction,
        help="Whether to print the agent configuration and exit.",
    )
    ...
    if args.print_model_config:
        print(agent.model)
        sys.exit(0)

    if args.print_agent_config:
        print(agent)
        sys.exit(0)
    ...
    if "CUDA" not in str(jax.devices()).upper():
        logger.warning("No CUDA device found, using %s", jax.devices()[0])
    ...
    if args.save:
        ...
        jnp.save(results_dir / "episodes.npy", jnp.array(episodes))
