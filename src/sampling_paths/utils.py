...
def random_scene(
    *,
    ...
    if sample_in_canyon:
        bounding_box = bounding_box.at[:, 1].apply(lambda y: jnp.clip(y, -7.5, 7.5))
    ...
    fill_factor = jr.uniform(
        key_fill_factor,
        (),
        minval=min_fill_factor,
        maxval=max_fill_factor,
    )
    scene = eqx.tree_at(
...
def validation_scene_keys(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        leave_progess: Whether to leave the progress bar after completion.
        key: The random key to be used.
        kwargs: Additional arguments to be passed to `random_scene`.
    ...
    if progress:
        it = trange(num_scenes, desc="Selecting validation scenes", leave=leave_progess)
    else:
        it = range(num_scenes)
