...
@pytest.fixture
def optim() -> optax.GradientTransformation:
    return optax.contrib.muon(1e-3)
...
def test_masked_actions(
    scene: TriangleScene, masked_actions: dict[int, Int[Array, " num_masked_actions"]]
) -> None:
    ...
    for order in [2, 3]:
        valid_paths = scene.compute_paths(order=order).masked()
        path_candidates = valid_paths.objects[:, 1:-1]
        for first_object, second_object in path_candidates[:, :2]:
            assert not jnp.isin(second_object, masked_actions[int(first_object)]), (
                ...
                f"but got path candidate: {path_candidates}"
            )
