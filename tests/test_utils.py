...
def test_geometric_transformation_invariance_properties(
    ...
    chex.assert_trees_all_close(
        ...
        xyz_t,
        atol=1e-5,
        custom_message="Translation invariance failed",
    )
    ...
    chex.assert_trees_all_close(
        ...
        xyz_s,
        atol=1e-5,
        custom_message="Scaling invariance failed",
    )
    ...
    expectation = (
        pytest.raises(AssertionError, match="Rotation invariance failed")
        if degenerate  # Rotation invariance will fail because TX-RX aligned with z-axis, so rotation along z-axis will change the scene
        else does_not_raise()
    )
    ...
    with expectation:
        chex.assert_trees_all_close(
            ...
            xyz_r,
            atol=1e-5,
            custom_message="Rotation invariance failed",
        )
    ...
    expectation = (
        pytest.raises(
            AssertionError, match="Combined transformations invariance failed"
        )
        if degenerate  # Rotation invariance will fail because TX-RX aligned with z-axis, so rotation along z-axis will change the scene
        else does_not_raise()
    )
